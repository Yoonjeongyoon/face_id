...
checkpoint = torch.load("mtlface_checkpoints.tar", map_location="cpu")

# checkpoint가 딕셔너리라면, 백본 관련 키만 추출합니다.
# 여기서는 키들이 "encoder."로 시작한다고 가정합니다.
backbone_state_dict = OrderedDict()
for key, value in checkpoint.items():
    if key.startswith("encoder."):
        new_key = key.replace("encoder.", "", 1)  # 접두어 제거
        backbone_state_dict[new_key] = value
model = MTLFace(input_size=112)
