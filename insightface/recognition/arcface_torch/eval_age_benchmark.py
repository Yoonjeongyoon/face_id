...
def distance(embeding1, embeding2, distance_metric = 0):
    ...
    if distance_metric ==0:
        ...
        norm2 = np.linalg.norm(embeding2, ord=2, axis=1)# 각 벡터의 절대 값의 곱
        norm = norm1 * norm2 + eps
    ...
    for fold_idx, (train_set, test_set) in enumerate(k_fold.split(indices)):
        ...
        if len(is_fp) > 0:
            ...
            for i, idx in enumerate(fp_indices):
                ...
                print(
                    f"  imp {idx - (int(len(test_set) / 2) * (fold_idx + 1))}: 거리 = {distance_val:.4f}, 임계값 = {best_threshold:.4f}, 차이 = {diff:.4f}")
                print(f"    경로1: {path1}")
                print(f"    경로2: {path2}")
        ...
        if len(is_fn) > 0:
            ...
            for i, idx in enumerate(fn_indices):
                ...
                print(
                    f"  gen {idx - (int(len(test_set) / 2) * fold_idx)}: 거리 = {distance_val:.4f}, 임계값 = {best_threshold:.4f}, 차이 = {diff:.4f}")
                print(f"    경로1: {path1}")
                print(f"    경로2: {path2}")
