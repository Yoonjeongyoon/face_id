    ...
    with torch.no_grad():
        ...
        for batch_imgs, batch_paths in dataloader:
            ...
            if isinstance(adapted_output, tuple):
                adapted_output_logits = adapted_output[0]
            else:
                adapted_output_logits = adapted_output
            adapted_embeds = adapted_output_logits.cpu().numpy()  # CPU로 변환 후 numpy 배열로 변환
...
def get_topofr_model(name='r50'):
    ...
    if name == 'r50':
        ...
        model_path = 'work_dirs/Glint360K_R50_TopoFR_9727.pt'
    elif name == 'r100':
        model_config = 'configs/glint360k_r100.py'
        model_path = 'work_dirs/Glint360K_R100_TopoFR_9760.pt'
    elif name == 'r200':
        model_config = 'configs/glint360k_r200.py'
        model_path = 'work_dirs/Glint360K_R200_TopoFR_9784.pt'
    else:
...
if __name__ == '__main__':
    ...
    log_file = os.path.join(os.path.dirname(__file__), 'log/CPLFW_motion_blur_3_benchmark_results.txt')
    # 콘솔과 파일에 동시 출력하도록 설정
        ...
        for tbs in batch_sizes:
            for epi in episodic:
                for ts in tent_steps:
                    ...
                    del embeddings_dict, adapted_embeddings_dict
                    del embeddings_eval, adapted_embeddings_eval
